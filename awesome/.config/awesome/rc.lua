...
local autostart_script = "autostart.sh"
local cleanup_script = "cleanup.sh"

-- Some widgets spawn background processes and they are not killed when Awesome
-- restarts, so they need to be cleaned up so they don't multiply
awful.spawn.with_shell(config_dir .. cleanup_script)
    ...
    naughty.notify(
        {
            ...
            title = "Error(s) encountered during startup",
...
terminal = "kitty"
floating_terminal = "kitty -class fkitty"
browser = "firefox"
editor = "vim" -- Emacs?
...
-- Notifications {{{
-- TODO: libnotify notifications don't respect certain options
--       naughty works as expected
...
awful.rules.rules = {
    ...
    -- Title bars always off
    -- Hide the title bars of these clients regardless of theme setting
    {
        rule_any = {
            class = {
                "Firefox",
    ...
    -- Title bars always on
    -- Show the title bars of these clients regardless of theme setting
    {
        rule_any = {
            class = {},
            type = {"dialog"},
    ...
    -----------------------------------------
    -- Start applications on specific tags --
    -----------------------------------------
    ...
    --     },
    --   },
    --   properties = { screen = 2, tag = tags[1] }
    -- },
    ...
    --     },
    --   }
    --   properties = { screen = 3, tag = tags[2] }
    -- },
}
...
-- If the layout is not floating, every floating client that appears should be centered
...
-- When a client starts up in full-screen mode, resize to cover the full screen
-- a short moment later. This fixes wrong geometry when titlebars are enabled
client.connect_signal(
    "manage",
    function(c)
        if c.fullscreen then
...
tag.connect_signal(
    ...
    function(t)
        for k, c in ipairs(t:clients()) do
            if awful.layout.get(mouse.screen) == awful.layout.suit.floating then
                -- Geometry x == 0 and y == 0 probably means that the clients
                -- have been spawned in a non-floating layout and thus they
                -- don't have their floating_geometry set properly. If that is
                -- the case, don't change their geometry
...
-- Enable rofi to unminimize minimized clients
client.connect_signal(
    "request::activate",
...
awful.spawn.with_shell(config_dir .. autostart_script)
